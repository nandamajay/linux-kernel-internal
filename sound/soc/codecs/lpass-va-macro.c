...
static int va_macro_setup_pm_clocks(struct device *dev, struct va_macro *va)
{
    ...
    /*
     * Keep ordering explicit so resume enables in a defined sequence.
     * (pm_clk list ordering is insertion order).
     */
	if (va->dcodec) {
	ret = pm_clk_add_clk(dev, va->dcodec);
	if (ret)
		return ret;
	}
	if (va->macro) {
	ret = pm_clk_add_clk(dev, va->macro);
	if (ret)
		return ret;
	}
	ret = pm_clk_add_clk(dev, va->mclk);
	if (ret)
		return ret;

	if (va->has_npl_clk && va->npl) {
		ret = pm_clk_add_clk(dev, va->npl);
	if (ret)
		return ret;
	}
